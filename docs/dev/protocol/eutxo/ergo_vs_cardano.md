...
The Ergo SDK represents the blockchain context for transaction signing with the [BlockchainStateContext](https://github.com/ergoplatform/sigmastate-interpreter/blob/develop/sdk/shared/src/main/scala/org/ergoplatform/sdk/wallet/protocol/context/BlockchainStateContext.scala) abstract class and its `CBlockchainStateContext` case class. The `sigmaLastHeaders` method provides information about the previous blocks, while `previousStateDigest` provides the UTXO set digest from the last header. The `sigmaPreHeader` method provides the current block's pre-header.
