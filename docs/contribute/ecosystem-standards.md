...
| **3.2** | The built-in spam protection should be sufficient if properly configured in `Server Settings -> Moderation -> Safety Setup` make sure all protections are enabled. Under `Automod` you can define custom words to filter an example would be `.gd, .gg, @everyone, #Support, adminsupports, bch.gg, Create Ticket, dsc.gg, earn $, https://t.me, Open-Ticket, Raise a ticket, shrtm.nu, Support Ticket, supportservice`. Alternatively you can enable [Wickbot](https://wickbot.com/). |
| **3.2** | The thread permission is often abused by scammers, luring users into private threads where they imitate support staff; disable threads entirely if unused, otherwise disable them on a per-channel or role basis. |
